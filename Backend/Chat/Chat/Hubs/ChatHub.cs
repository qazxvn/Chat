...
public class ChatHub : Hub<IChatClient>
{
    ...
    public async Task JoinChat(UserConnection connection)
    {
        ...
        await Clients.Group(connection.ChatRoom).ReceiveMessage("Admin", $"{connection.UserName} вошел в чат");
    }
    ...
    public override async Task OnDisconnectedAsync(Exception? exception)
    {
        ...
        if (connection is not null)
        {
            ...
            await Clients.Group(connection.ChatRoom).ReceiveMessage("Admin", $"{connection.ChatRoom} вышел из чата");
        }
    }
}